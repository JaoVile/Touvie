...
const [accs, cats, recs, bills, txCount] = await Promise.all([
  ...
  admin
    .from("transactions")
    .select("description,amount_cents,kind,recurrence_rule,reminder_enabled")
    .eq("user_id", uid)
    .eq("is_recurring", true),
  admin
    .from("bills")
    .select("title,amount_cents,due_date,paid_at,recurrence_rule")
    .eq("user_id", uid)
    .order("due_date"),
...
for (const a of accs.data ?? [])
  console.log(`   ${a.archived ? "🗄️ " : ""}${a.name} [${a.kind}] = ${brl(a.balance_cents)}`);

console.log(`\n🏷️  CATEGORIAS (${cats.data?.length ?? 0}):`);
for (const c of cats.data ?? [])
  console.log(`   ${c.emoji ?? ""} ${c.name} (${c.kind})${c.archived ? " 🗄️" : ""}`);

console.log(`\n🔁 RECORRENTES (${recs.data?.length ?? 0}):`);
for (const r of recs.data ?? [])
  console.log(
    `   ${r.kind === "income" ? "➕" : "➖"} ${r.description ?? "—"} ${brl(r.amount_cents)} [${r.recurrence_rule ?? "?"}]${r.reminder_enabled ? " 🔔" : ""}`,
  );

console.log(`\n📅 BILLS / contas a vencer (${bills.data?.length ?? 0}):`);
for (const b of bills.data ?? [])
  console.log(
    `   ${b.paid_at ? "✅" : "⬜"} ${b.due_date} · ${b.title} · ${brl(b.amount_cents)}${b.recurrence_rule ? ` [${b.recurrence_rule}]` : ""}`,
  );
