...
> **Status:** MVP IMPLEMENTADO (jul/2026, commit `feat(touvi)`). Uma **seção nova**
> do app logado, ao lado de Treinos / Finanças / Notas — a diferença é que em vez
> de formulário é um **chat** com um assistente de IA chamado **Touvi**.
...
- **Usar o `glm-4.7-flash`** — **GRÁTIS** na mesma API da Z.ai. Subir pro pago
  depois = trocar **uma string** (`MODEL` em `lib/touvi.ts`).
...
2. ✅ `lib/touvi.ts` — wrapper do GLM (`fetch` → Z.ai, `MODEL = "glm-4.7-flash"`,
   key em `ZAI_API_KEY`) + o *system prompt* da persona (`TOUVI_SYSTEM`).
3. ✅ `app/api/touvi/route.ts` — `getUser()` (401 se anônimo), valida a mensagem
   (zod, 1–4000 chars), grava a do usuário, manda as últimas `HISTORY_WINDOW = 20`
   ao modelo, grava a resposta, devolve `{ reply }`.
4. ✅ `app/(app)/touvi/page.tsx` + `TouviChat.tsx` — Server Component carrega o
...
Dar ao Touvi acesso de **leitura** a resumos de finanças / rotina / metas como
...
- [ ] **Persona** — o `TOUVI_SYSTEM` está funcional; afinar tom / o que ele NÃO faz.
