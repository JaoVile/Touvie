...
const { data: existingAccs } = await db.from("finance_accounts").select("id,name,kind,balance_cents").eq("user_id", uid);
...
for (const a of wantAccounts) {
  ...
  if (found) {
    if (found.balance_cents !== reais(a.balance)) {
      await db.from("finance_accounts").update({ balance_cents: reais(a.balance) }).eq("id", found.id);
      console.log(`🏦 atualizado saldo ${a.name}: ${brl(reais(a.balance))}`);
    } else {
      console.log(`🏦 ok (já existe): ${a.name} = ${brl(found.balance_cents)}`);
    }
  } else {
    await db.from("finance_accounts").insert({ user_id: uid, name: a.name, kind: a.kind, balance_cents: reais(a.balance) });
    console.log(`🏦 criada conta: ${a.name} [${a.kind}] = ${brl(reais(a.balance))}`);
  }
}

// --- 2. CATEGORIAS (mapa nome -> id) --------------------------------
const { data: cats } = await db.from("finance_categories").select("id,name,kind").eq("user_id", uid).eq("archived", false);
...
for (const b of bills) {
  ...
  if (billKey.has(`${b.title}|${due}`)) { console.log(`📅 ok (já existe): ${due} ${b.title}`); continue; }
  ...
  if (error) { console.log(`❌ ${b.title}: ${error.message}`); continue; }
